...
// Bundled demo case (auto-loaded on mount). `pre` is the intact pre-treatment
// scan; `tx` is the prepared (with-ditch) scan the QC runs against.
...
export function PrepEntry({
  ...
  // Bundled case auto-load: fetch the default pre-treatment / with-ditch scans
  // once on mount so the tool opens ready to use. Clearing a chip lets the
  // user drop in their own PLY instead; we never re-fetch after that.
  const [defaultsLoading, setDefaultsLoading] = useState(true);
  const defaultsRequested = useRef(false);
  useEffect(() => {
    if (defaultsRequested.current) return;
    ...
    (async () => {
      try {
        ...
        const [preGeom, txGeom] = await Promise.all([
          load(defaultPreUrl),
          load(defaultTxUrl),
        ]);
        if (cancelled) return;
        setPre({ name: "Pre-treatment (bundled)", geometry: preGeom });
        setTx({ name: "Treatment scan (bundled)", geometry: txGeom });
      } catch (err) {
  ...
  const SECTION_PALETTE = [
    0x00ADEF, 0xe0731a, 0x7d3ac1, 0x1c7d4d, 0xc41f1f,
  ];
  ...
  return (
    <div className="flex h-full w-full flex-col bg-[#D6E7F1] font-['Roboto',sans-serif] text-[#3e3d40] antialiased">
      {inIframe && !hasBoth && (
        <a
      ...
      {/* Header */}
      <header className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-b border-[#d1d1d1] bg-white px-5 py-3 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-md bg-[#00ADEF] text-white">
          ...
          <div>
            ...
            <div className="text-[11.5px] text-[#818181]">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            <>
              <button
                ...
                className={[
                  ...
                  sectionStatus === "ok"
                    ? "border-[#d1d1d1] bg-white text-[#3e3d40] hover:bg-[#f4f7fa]"
          ...
          <button
            ...
            className={[
              ...
              hasBoth && autoStatus.state !== "running"
                ...
                : "cursor-not-allowed border-[#d1d1d1] bg-[#e6eef7] text-[#7a8597]",
            ].join(" ")}
          ...
          <div className="relative">
            ...
            {pasteOpen && hasBoth && (
              <div className="absolute right-0 top-full z-30 mt-2 w-[340px] rounded-lg border border-[#d1d1d1] bg-white p-3 shadow-xl">
                <div className="mb-1.5 text-[11.5px] font-semibold text-[#3e3d40]">
                  Apply 4×4 rigid transform
                </div>
                <div className="mb-2 text-[11px] leading-snug text-[#818181]">
                ...
                <textarea
                  ...
                  rows={5}
                  spellCheck={false}
                  className="w-full resize-none rounded-md border border-[#d1d1d1] bg-[#f8fafc] px-2 py-1.5 font-mono text-[11px] leading-relaxed text-[#3e3d40] focus:border-[#00ADEF] focus:outline-none"
                />
                {pasteError && (
                ...
                <div className="mt-2 flex items-center justify-end gap-1.5">
                  <button
                    ...
                    className="rounded-md border border-[#d1d1d1] bg-white px-2.5 py-1 text-[11.5px] font-medium text-[#3e3d40] hover:bg-[#f4f7fa]"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="rounded-md border border-[#00ADEF] bg-[#00ADEF] px-2.5 py-1 text-[11.5px] font-semibold text-white hover:bg-[#0095CE]"
                  >
                    Apply
                  </button>
                </div>
          ...
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="ml-2 flex h-8 w-8 items-center justify-center rounded-md border border-[#d1d1d1] bg-white text-[#3e3d40] transition hover:bg-[#f4f7fa]"
        ...
        {hasBoth ? (
          <>
            <div className="relative flex-1">
              ...
              {!canGoToApp && (
                <div className="absolute right-4 top-4 w-[238px] rounded-lg bg-white/95 p-3 shadow-xl ring-1 ring-[#d1d1d1] backdrop-blur">
                  <div className="mb-2 flex items-center gap-2">
                    <ArrowRight className="h-4 w-4 text-[#1c7d4d]" />
                    <div className="text-[12.5px] font-semibold text-[#3e3d40]">
                      Before "Go to App"
                    </div>
                  </div>
                  <div className="space-y-1">
                    {handoffChecklist.map((c) => (
                      ...
                      >
                        <span
                          className={[
                            ...
                            c.done
                              ...
                              : "border border-[#d1d1d1] text-[#a0a8b4]",
                          ].join(" ")}
                        >
                          {c.done ? "✓" : ""}
                        </span>
                        <span
                          className={
                            c.done ? "text-[#818181]" : "text-[#3e3d40]"
                          }
                        >
                          {c.label}
                        </span>
              ...
              <div className="pointer-events-none absolute bottom-4 left-4 flex flex-col items-start gap-1">
                <span
                  ref={scaleLabelRef}
                  className="rounded bg-white/85 px-1.5 py-0.5 font-mono text-[10.5px] font-medium text-[#3e3d40] shadow-sm ring-1 ring-[#d1d1d1]"
                >
                  1 mm
                </span>
                <div
                  ref={scaleBarRef}
                  className="h-[6px] border-x-2 border-b-2 border-[#3e3d40]"
                  style={{ width: "96px" }}
                />
              </div>
              <Legend
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-3">
            <Loader2 className="h-8 w-8 animate-spin text-[#00ADEF]" />
            <div className="text-[13px] font-medium text-[#818181]">
              Loading scans…
            </div>
          </div>
        ) : (
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <div className="text-[10.5px] font-semibold uppercase tracking-wide text-[#818181]">
          {label}
        </div>
        <div className="max-w-[180px] truncate text-[12px] font-medium text-[#3e3d40]">
          {value.name}
        </div>
      ...
      <button
        onClick={onClear}
        className="ml-1 flex h-6 w-6 items-center justify-center rounded text-[#8a93a3] hover:bg-[#f0f3f7] hover:text-[#3e3d40]"
        title="Remove"
      >
...
function DropZone({
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <div className="mt-3 text-[14px] font-semibold text-[#3e3d40]">
            {label} loaded
          </div>
          <div className="mt-1 text-[12px] text-[#818181]">
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 text-[18px] font-semibold text-[#3e3d40]">
        Drop {label.toLowerCase()} here
      </div>
      <div className="mt-1 text-[13px] text-[#818181]">{sublabel}</div>
      <div className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-[#f4f7fa] px-3 py-1 text-[11.5px] text-[#818181]">
...
function HelpCard({ mode }: { mode: ViewMode }) {
  ...
  return (
    <div className="absolute right-4 top-4 max-w-[280px] rounded-md bg-white/90 px-3 py-2 text-[11.5px] leading-snug text-[#818181] shadow-md ring-1 ring-[#d1d1d1] backdrop-blur">
      {mode === "align" ? (
        <>
          <div className="mb-0.5 text-[11px] font-semibold uppercase tracking-wide text-[#3e3d40]">
            Align mode
          </div>
...
function LabelPanel({
  ...
  const ready = total > 0;
  return (
    <div className="absolute left-4 top-[200px] w-[280px] rounded-lg bg-white/95 p-3 shadow-xl ring-1 ring-[#d1d1d1] backdrop-blur">
      <div className="mb-2 flex items-center gap-2">
      ...
      <div className="mb-2 text-[10.5px] font-semibold uppercase tracking-wide text-[#818181]">
        Brush
      </div>
      ...
      <div className="mb-3">
        <div className="mb-1 flex justify-between text-[10.5px] text-[#818181]">
      ...
      <div className="mb-2 space-y-1">
        <div className="flex items-center justify-between">
          <div className="text-[11px] font-semibold text-[#818181]">
            Marked vertices
          </div>
          <button
            onClick={onClear}
            className="flex items-center gap-1 rounded px-1.5 py-0.5 text-[11px] text-[#8a93a3] hover:bg-[#f0f3f7] hover:text-[#3e3d40]"
  ...
  return (
    <div className="rounded-md border border-[#e2e7ee] bg-[#f7f9fc] px-1 py-1">
      <div className="flex items-center justify-center gap-1">
        ...
        />
        <span className="text-[10px] font-medium text-[#818181]">{label}</span>
      </div>
      <div className="tabular-nums text-[11px] font-semibold text-[#3e3d40]">
...
function SectionControlPanel({
  ...
  const MAX = 120;
  return (
    <div className="absolute left-4 top-[182px] w-[230px] rounded-lg bg-white/95 p-3 shadow-xl ring-1 ring-[#d1d1d1] backdrop-blur">
      <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-[#3e3d40]">
        Anatomical plane
      </div>
      <div className="mb-3 grid grid-cols-3 overflow-hidden rounded-md border border-[#d1d1d1]">
        {([
      ...
      <div className="mb-3 flex overflow-hidden rounded-md border border-[#d1d1d1]">
        {([
...
function UndercutControlPanel({
  ...
  return (
    <div className="absolute left-4 top-[182px] w-[238px] rounded-lg bg-white/95 p-3 shadow-xl ring-1 ring-[#d1d1d1] backdrop-blur">
      <div className="mb-2 flex items-center gap-2">
        <Magnet className="h-4 w-4 text-[#00ADEF]" />
        <div className="text-[12.5px] font-semibold text-[#3e3d40]">
          Insertion path
        </div>
      </div>
...
function HeatmapControlPanel({
  ...
  return (
    <div className="absolute left-4 top-[182px] w-[248px] rounded-lg bg-white/95 p-3 shadow-xl ring-1 ring-[#d1d1d1] backdrop-blur">
      <div className="mb-2 flex items-center gap-2">
        <Flame className="h-4 w-4 text-[#00ADEF]" />
        <div className="text-[12.5px] font-semibold text-[#3e3d40]">
          Reduction clearance
        </div>
      </div>

      <div className="mb-1 text-[10.5px] font-semibold uppercase tracking-wide text-[#818181]">
        Material
      </div>
      <select
        value={materialId}
        onChange={(e) => onMaterialId(e.target.value)}
        className="mb-3 w-full rounded-md border border-[#d1d1d1] bg-white px-2 py-1.5 text-[12px] text-[#3e3d40]"
      >
        {MATERIALS.map((m) => (
      ...
      <div className="mb-2 flex overflow-hidden rounded-md border border-[#d1d1d1]">
        {([
          { value: "all", label: "All" },
      ...
      <label className="mb-3 flex cursor-pointer items-center gap-2 text-[11px] text-[#818181]">
        <input
          type="checkbox"
...
function SavedCutsPanel({
  ...
  return (
    <div className="absolute bottom-4 left-4 w-[238px] rounded-lg bg-white/95 p-3 shadow-xl ring-1 ring-[#d1d1d1] backdrop-blur">
      <div className="mb-2 flex items-center gap-2">
        <Scissors className="h-4 w-4 text-[#00ADEF]" />
        <div className="text-[12.5px] font-semibold text-[#3e3d40]">
          Saved cuts
        </div>
      </div>
      <div className="space-y-1">
        {saved.map((s, i) => (
          ...
          >
            ...
            />
            <span className="flex-1 text-[12px] font-medium text-[#3e3d40]">
              Cut {i + 1}
            </span>
            <button
              type="button"
...
function OpacityPanel({
  ...
  return (
    <div className="absolute left-4 top-4 w-[210px] rounded-lg bg-white/95 p-3 shadow-xl ring-1 ring-[#d1d1d1] backdrop-blur">
      <OpacityRow
        label="Treatment scan"
        opacity={txOpacity}
...
function OpacityRow({
  ...
  return (
    <div>
      <div className="mb-1 text-[11px] font-medium text-[#3e3d40]">{label}</div>
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="rounded p-1 text-[#00ADEF] hover:bg-[#eef4fb]"
...
function ToothGlyph({ muted }: { muted?: boolean }) {
  const stroke = muted ? "#b3bdcb" : "#3e3d40";
  return (
    <svg
      width="20"
      height="14"
...
function makeMeasurementLabel(
  ...
  const line = new THREE.Line(
    lineGeom,
    new THREE.LineBasicMaterial({ color: 0x3e3d40, depthTest: false }),
  );
