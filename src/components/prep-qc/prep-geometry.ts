...
// ---------------------------------------------------------------------------
// Margin line — trace the finish line of the prep as a closed loop. The
// marginal-labeled band is binned by angle around the prep center (in the
// prep's own in-plane frame); the outermost vertex per angular bin is the
// finish-line point. Returned as a bright, depth-test-off LineLoop floated
// just above the surface so it reads clearly over the heatmap.
// ---------------------------------------------------------------------------
export function buildMarginLine(
  ...
  const pos = geom.attributes.position;
  if (!pos) return null;
  const u = frame.longDir.clone().normalize();
  const v = frame.crossDir.clone().normalize();
  const n = frame.up.clone().normalize();
  const BINS = 96;
  const best: ({ r: number; p: THREE.Vector3 } | null)[] = new Array(BINS).fill(
    null,
  );
  const w = new THREE.Vector3();
  const d = new THREE.Vector3();
  const count = Math.min(labels.length, pos.count);
  for (let i = 0; i < count; i++) {
    if (labels[i] !== LABEL_MARGINAL) continue;
    w.fromBufferAttribute(pos, i).applyMatrix4(matrix);
    d.copy(w).sub(frame.center);
    const a = d.dot(u);
    const b = d.dot(v);
    const ang = Math.atan2(b, a);
    const bin = Math.min(
      BINS - 1,
      Math.max(0, Math.floor(((ang + Math.PI) / (2 * Math.PI)) * BINS)),
    );
    const r = Math.hypot(a, b);
    const cur = best[bin];
    if (!cur || r > cur.r) best[bin] = { r, p: w.clone() };
  }
  const pts: THREE.Vector3[] = [];
  for (let i = 0; i < BINS; i++) {
    const cell = best[i];
    if (cell) pts.push(cell.p.addScaledVector(n, 0.15));
  }
